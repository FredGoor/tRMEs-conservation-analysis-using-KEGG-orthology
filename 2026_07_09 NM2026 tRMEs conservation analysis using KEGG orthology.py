# -*- coding: utf-8 -*-
"""
Conservation of MiaA, MnmE and MnmG across bacterial phyla using KEGG + NCBI.

This script avoids genome FASTA downloads. It asks a simple question:
for each bacterial phylum represented by KEGG bacterial genome representatives,
what fraction encode an annotated ortholog of MiaA, MnmE or MnmG?

Data sources used online:
    https://rest.kegg.jp/list/genome/bacteria
    https://rest.kegg.jp/link/taxonomy/genome
    https://rest.kegg.jp/link/genes/K00791
    https://rest.kegg.jp/link/genes/K03650
    https://rest.kegg.jp/link/genes/K03495
    NCBI Taxonomy EFetch, to convert NCBI taxids into phylum names.

Manual mode is also supported. Place these files in a folder called
manual_kegg_files next to this script:
...
OUTPUT_BASE_DIR = r"C:\Dropbox\Publications\2026\NM2026 Dirk tRMEs\2026 Final computational analyses\2026-07-07 Conservation of tRMEs across bacteria"
OUTPUT_FOLDER_PREFIX = "kegg_tRNA_modifier_conservation"
CREATE_TIMESTAMPED_OUTPUT_FOLDER = True
CACHE_FOLDER_NAME = "_cache"
MANUAL_INPUT_FOLDER_NAME = "manual_kegg_files"
...
HEADERS = {
    "User-Agent": (
        ...
        "Chrome/120.0 Safari/537.36 tRNA-modifier-conservation-script"
    ),
...
def manual_input_folder() -> Path:
    return script_folder() / MANUAL_INPUT_FOLDER_NAME
...
def download_text_from_kegg(endpoint: str) -> str:
    ...
    if response.status_code != 200 or not response.text.strip():
        ...
        raise RuntimeError(
            ...
            "If this works in your browser but not in Python, use DATA_SOURCE_MODE = 'manual'."
        )
...
def main():
    ...
    print("\nDone.")
    print("Use 'phylum_summary' for exact values and 'genome_presence' to inspect individual genomes.")
